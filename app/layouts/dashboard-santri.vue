          ...
          <UDropdownMenu :items="dropdownItems" :ui="{ content: 'min-w-52' }">
            <UButton variant="ghost" trailing-icon="i-ph-caret-down">
              <UAvatar
                ...
                :alt="(auth.user.value?.nickname || auth.user.value?.fullname) ?? ''"
                :text="getInitials((auth.user.value?.nickname || auth.user.value?.fullname))"
                class="bg-primary text-white! text-xs rounded-full"
              />
              <span class="text-sm font-medium">{{
                (auth.user.value?.nickname || auth.user.value?.fullname)
              }}</span>
            </UButton>
          </UDropdownMenu>
        </div>
