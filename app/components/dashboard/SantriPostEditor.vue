...
const { data: metaData } = await useFetch<{ categories: CategoryOption[] }>('/api/dashboard/santri/posts/meta', {
  key: 'dashboard-santri-posts-meta',
})

const { data: postData, status: postStatus } = await useFetch<{ data: EditorPost }>(
  () => isEdit.value ? `/api/dashboard/santri/posts/${props.postId}` : null,
  {
    key: () => isEdit.value ? `dashboard-santri-post-editor:${props.postId}` : 'dashboard-santri-post-editor:create',
...
const categoryItems = computed(() =>
  (metaData.value?.categories ?? []).map(category => ({
    label: category.type === 'berita' ? `Berita · ${category.name}` : `Pena Santri · ${category.name}`,
    value: category.id,
  })),
)
...
const canSubmit = computed(() => Boolean(form.featuredImage && form.categoryId))
...
const wordCount = computed(() => plainTextContent.value ? plainTextContent.value.split(' ').length : 0)
const readingTime = computed(() => wordCount.value > 0 ? Math.ceil(wordCount.value / 200) : 0)
...
async function saveDraft({ silent = false, redirectAfterCreate = true, manageLoading = true } = {}) {
  ...
  try {
    ...
    const response = isEdit.value
      ? await $fetch<{ id: number, status: EditorPost['status'] }>(`/api/dashboard/santri/posts/${props.postId}`, {
        method: 'PATCH',
        body: payload,
      })
      : await $fetch<{ id: number, status: EditorPost['status'] }>('/api/dashboard/santri/posts', {
        method: 'POST',
        body: payload,
      })
...
async function submitReview() {
  ...
  try {
    ...
    const response = await $fetch<{ status: EditorPost['status'] }>(`/api/dashboard/santri/posts/${postId}/submit`, {
      method: 'POST',
      body: payload,
    })
    ...
    toast.add({
      title: currentStatus.value === 'pending_review' ? 'Post dikirim untuk review' : 'Post diperbarui',
      color: 'success',
      icon: 'i-lucide-send',
    })
...
const backLabel = computed(() => isEdit.value ? 'Edit Post' : 'Tulis Post Baru')
</script>

<template>
  <div class="min-h-[calc(100vh-60px)] bg-white">
    <div class="sticky top-[60px] z-20 border-b border-neutral-200 bg-white">
      <div class="flex min-h-[48px] flex-col gap-3 px-4 py-3 md:flex-row md:items-center md:justify-between md:px-5 md:py-0">
        <UButton
          to="/dashboard/posts"
          variant="ghost"
        ...
        <UBadge :color="statusColor" variant="subtle" class="self-start md:self-auto">
          {{ statusLabel }}
        </UBadge>
        ...
        <div class="flex flex-col gap-2 sm:flex-row">
          ...
          <UButton
            ...
            @click="submitReview()"
          >
            {{ currentStatus === 'rejected' ? 'Kirim Ulang' : 'Kirim untuk Review' }}
          </UButton>
        </div>
      </div>
    </div>
        ...
        <div v-else class="space-y-4">
          <div>
            <input
              ...
              class="w-full border-0 bg-transparent text-2xl font-bold outline-none placeholder:text-neutral-300"
            >
          ...
          <div class="border-t border-neutral-200 pt-4">
            <UEditor
              ...
              class="min-h-[420px] rounded-xl border border-neutral-200 bg-white"
            >
              <template #default="{ editor }">
                <UEditorToolbar
                  :editor="editor"
                  :items="toolbarItems"
                  class="border-b border-neutral-200 px-3 py-2"
                />
              </template>
            </UEditor>
          </div>

          <p class="border-t border-neutral-200 pt-3 text-xs text-dimmed">
            {{ wordCount }} kata · ~{{ readingTime }} menit baca
          </p>
        </div>
      </div>

      <aside class="w-full border-t border-neutral-200 bg-neutral-50 p-4 lg:w-[320px] lg:border-t-0 lg:border-l lg:p-5">
        <div class="space-y-5">
          <UAlert
          ...
          <div class="space-y-2">
            ...
            <div v-if="form.featuredImage" class="overflow-hidden rounded-xl border border-neutral-200 bg-white">
              <img :src="form.featuredImage" alt="Preview cover post" class="h-40 w-full object-cover">
            </div>
          </div>
          ...
          <div class="space-y-2">
            <div class="flex items-center justify-between gap-2">
              ...
              <span class="text-xs text-dimmed">{{ form.excerpt.length }}/200</span>
            </div>
            <UTextarea
            ...
            <div class="rounded-xl border border-neutral-200 bg-white p-3">
              <div v-if="form.tags.length" class="mb-2 flex flex-wrap gap-2">
                <span
                  ...
                  :key="tag"
                  class="inline-flex items-center gap-1 rounded-full bg-neutral-100 px-2.5 py-1 text-xs font-medium text-neutral-700"
                >
                  {{ tag }}
                  <button type="button" class="text-dimmed hover:text-neutral-900" @click="removeTag(tag)">
                    ×
                  </button>
                </span>
              </div>
              ...
              <input
                ...
                class="w-full border-0 bg-transparent text-sm outline-none placeholder:text-neutral-400"
