...
async function save() {
  ...
  try {
    ...
    if (editingId.value !== null) {
      ...
      toast.add({ title: 'Kategori diperbarui', color: 'success', icon: 'i-ph-check-circle' })
    } else {
      await $fetch('/api/categories', { method: 'POST', body })
      toast.add({ title: 'Kategori ditambahkan', color: 'success', icon: 'i-ph-check-circle' })
    }
...
async function doDelete() {
  ...
  try {
    ...
    toast.add({ title: 'Kategori dihapus', color: 'success', icon: 'i-ph-check-circle' })
...
<template>
  <AdminDataTable :data="filteredCategories" :columns="columns">
    <template #toolbar-left>
      <UInput v-model="search" placeholder="Cari kategori…" icon="i-ph-magnifying-glass-bold" class="w-56" />
    </template>
  ...
  <UModal v-model:open="isDeleteModalOpen" title="Hapus Kategori">
    <template #body>
      <p class="text-sm">Apakah kamu yakin ingin menghapus kategori ini? Kategori tidak bisa dihapus jika masih digunakan oleh artikel.</p>
    </template>
    <template #footer>
      <div class="flex justify-end gap-2">
