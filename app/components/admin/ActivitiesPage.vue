...
type Activity = { id: number, title: string, description: string, imagePath: string, order: number }
...
function openEdit(item: Activity) {
  ...
  Object.assign(form, { title: item.title, description: item.description, imagePath: item.imagePath, order: item.order })
  uploadFile.value = null
...
async function save() {
  ...
  try {
    ...
    if (uploadFile.value) {
      ...
      const { path } = await $fetch<{ path: string }>('/api/activities/upload', {
        method: 'POST',
        body: formData,
      })
      finalImagePath = path
...
const columns: TableColumn<Activity>[] = [
  {
    accessorKey: 'imagePath', header: 'Foto', cell: ({ row }) =>
      h('img', { src: row.original.imagePath, alt: row.original.title, class: 'h-12 w-16 rounded object-cover' }),
  },
  { accessorKey: 'title', header: 'Judul', cell: ({ row }) => h('span', { class: 'font-medium' }, row.original.title) },
...
<template>
  <div>
    <AdminDataTable :data="activities" :columns="columns">
      <template #toolbar-right>
        <UButton label="Tambah Kegiatan" icon="i-ph-plus-bold" @click="openCreate" />
      </template>
    </AdminDataTable>

    <UModal v-model:open="isModalOpen" :title="editingId ? 'Edit Kegiatan' : 'Tambah Kegiatan'">
      <template #body>
        <div class="space-y-4">
    ...
    <UModal v-model:open="isDeleteModalOpen" title="Hapus Kegiatan">
      <template #body>
        <p class="text-sm">Apakah kamu yakin ingin menghapus kegiatan ini? Tindakan ini tidak bisa dibatalkan.</p>
      </template>
      <template #footer>
        <div class="flex justify-end gap-2">
