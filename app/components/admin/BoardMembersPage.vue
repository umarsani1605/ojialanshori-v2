...
type BoardMember = { id: number, name: string, role: string, avatarPath: string | null, order: number }

const ROLES = ['Penasehat', 'Pengajar']

const toast = useToast()
const { data, refresh } = useFetch<{ data: BoardMember[] }>('/api/board-members')
...
function openEdit(item: BoardMember) {
  ...
  Object.assign(form, { name: item.name, role: item.role, avatarPath: item.avatarPath || '', order: item.order })
  uploadFile.value = null
...
async function save() {
  ...
  try {
    ...
    if (uploadFile.value) {
      ...
      const { path } = await $fetch<{ path: string }>('/api/board-members/upload', {
        method: 'POST',
        body: formData,
      })
      finalImagePath = path
    ...
    const url = editingId.value ? `/api/board-members/${editingId.value}` : '/api/board-members'
...
async function doDelete() {
  ...
  deleting.value = true
  try {
    await $fetch(`/api/board-members/${deletingId.value}`, { method: 'DELETE' })
...
const columns: TableColumn<BoardMember>[] = [
  {
    accessorKey: 'avatarPath', header: 'Foto', cell: ({ row }) => {
       if (row.original.avatarPath) {
         return h('img', { src: row.original.avatarPath, alt: row.original.name, class: 'size-10 rounded-full object-cover' })
       }
...
<template>
  <div>
    <AdminDataTable :data="members" :columns="columns">
      <template #toolbar-right>
        <UButton label="Tambah Pengurus" icon="i-ph-plus-bold" @click="openCreate" />
      </template>
    </AdminDataTable>

    <UModal v-model:open="isModalOpen" :title="editingId ? 'Edit Pengurus' : 'Tambah Pengurus'">
      <template #body>
        <div class="space-y-4">
    ...
    <UModal v-model:open="isDeleteModalOpen" title="Hapus Pengurus">
      <template #body>
        <p class="text-sm">Apakah kamu yakin ingin menghapus data pengurus ini? Tindakan ini tidak bisa dibatalkan.</p>
      </template>
      <template #footer>
        <div class="flex justify-end gap-2">
